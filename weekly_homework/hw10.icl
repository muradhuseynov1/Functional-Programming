...
studentGPA std = (toReal(sum [a\\a<-std.grades]))/(toReal(length [a\\a<-std.grades]))
...
highestGPA :: [Student] {Student} -> University
highestGPA [x] _ = x.uni
highestGPA [x,y:xs] a
| overallGPAuni x.uni a > overallGPAuni y.uni a = highestGPA [x:xs] a
= highestGPA [y:xs] a

overallGPAuni :: University {Student} -> Real
overallGPAuni u arr = toReal(sum grades) / toReal(length grades)
where
    std = filter (\a = a.uni == u) (arrToList arr)
    grades = flatten (map (\a = a.grades) std)


uniWithHighestAverage :: {Student} -> University
uniWithHighestAverage std = highestGPA ArrayToList std
where
    ArrayToList = arrToList std
